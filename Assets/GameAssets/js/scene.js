...
export function initScene() {
  ...
  new GLTFLoader().load(
    "Assets/GameAssets/models/glb/firstiteration.glb",
    function (gltf) {
      gltf.scene.traverse(function (object) {
...
function addRenderTargets() {
  ...
  for (let i = 0; i < sceneCount; i++) {
    ...
    if (i === 0) {
      ...
      targetPlane.rotation.y = -Math.PI;
      targetPlane.position.set(0, 3.5, -newHeight / divSize);
    } else if (i === 2) {
      targetPlane = new THREE.Mesh(
        new THREE.PlaneGeometry(newWidth, newHeight, 32),
        materials[i]
      );
      targetPlane.rotation.y = Math.PI / 2;
      targetPlane.position.set(newWidth / divSize, 3.5, 0);
    } else if (i === 3) {
      targetPlane = new THREE.Mesh(
        new THREE.PlaneGeometry(newWidth, newHeight, 32),
        materials[i]
    ...
    new GLTFLoader().load(
      "Assets/GameAssets/models/glb/testround.glb",
      function (gltf) {
        gltf.scene.traverse(function (object) {
          object.receiveShadow = true;
        });
  ...
  const textMaterial = new THREE.MeshBasicMaterial({ color: 0xff33cc }); // Pink text color
